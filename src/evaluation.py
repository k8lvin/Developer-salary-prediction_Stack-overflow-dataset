...
def print_observations(metrics: dict):
    ...
    print(f"MAE of ${mae} means our model's average is off by ${mae:,.0f} from the true salary")
    
    if r2 > 0.7:
        print(f'r2 score of {r2:.3f} is strong - the model explains {r2 * 100}% of the variance in salary')
    elif r2 > 0.5:
        print(f"r2 of {r2:.3f} is moderate- there is still variance the model cannot capture (Expected for salary data)")
    else:
